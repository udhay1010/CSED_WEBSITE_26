...
export default function KineticTitle({ startAnimation = false }) {
  ...
  useEffect(() => {
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        if (aboutTopRef.current) aboutTopRef.current.classList.add("neon-flicker-red");
        if (aboutBottomRef.current) aboutBottomRef.current.classList.add("neon-flicker-red");
        if (csedRef.current) csedRef.current.classList.add("neon-flicker-black");
  ...
  return (
    <div ref={containerRef} className="flex items-center gap-6 font-bold text-5xl md:text-7xl tracking-widest uppercase cursor-default">
      ...
      <div className="relative flex flex-col items-center justify-center overflow-hidden">
        ...
        >
          <span
            className="text-black text-2xl md:text-4xl font-black tracking-[0.4em]"
            spellCheck={false}
            autoCorrect="off"
            autoComplete="off"
